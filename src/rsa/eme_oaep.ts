 ...
 * @param algorithm
 */
export async function eme_oaep_encode(
  label: Uint8Array,
  m: Uint8Array,
  k: number,
  algorithm: RSAHashAlgorithm,
): Promise<Uint8Array> {
  const labelHash = new Uint8Array(await digest(algorithm, label));
  ...
  const dbMask = await mgf1(seed, k - labelHash.length - 1, algorithm);
  const maskedDb = xor(db, dbMask);
  const seedMask = await mgf1(maskedDb, labelHash.length, algorithm);
  const maskedSeed = xor(seed, seedMask);

  return concat([0x00], maskedSeed, maskedDb);
}

export async function eme_oaep_decode(
  label: Uint8Array,
  c: Uint8Array,
  k: number,
  algorithm: RSAHashAlgorithm,
